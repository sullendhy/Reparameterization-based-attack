...
def save_to_file(file_name, contents):
    with open(file_name, 'a', errors = 'ignore') as fh:
...
def main(args):
    ...
    if args.constraint == "bertscore_idf":
        if args.dataset == 'mnli':
            #idf_dict = get_idf_dict(dataset['train']['premise'] + dataset['train']['hypothesis'], tokenizer, nthreads=20)#原线程是20
            idf_dict = get_idf_dict(dataset['train']['premise'] + dataset['train']['hypothesis'], tokenizer, nthreads=1)
        else:
            #idf_dict = get_idf_dict(dataset['train'][text_key], tokenizer, nthreads=20)
            idf_dict = get_idf_dict(dataset['train'][text_key], tokenizer, nthreads=1)
    ...
    for idx in range(args.start_index, end_index):
        ...
        if args.model == 'gpt2' or 'roberta-base':
            token_type_ids = None
        else:
        ...
        with torch.no_grad():
            ...
            if args.constraint.startswith('bertscore'):
                ...
                #if args.model == 'gpt2' or 'bert-base-uncased' in args.model: 原约束
        ...
        for i in range(args.num_iters):
            optimizer.zero_grad()
            # coeffs = F.gumbel_softmax(log_coeffs.unsqueeze(0).repeat(args.batch_size, 1, 1), hard=False) # B x T x V 原来的是False
            coeffs = F.gumbel_softmax(log_coeffs.unsqueeze(0).repeat(args.batch_size, 1, 1), hard=True) # B x T x V
            ...
            if args.lam_sim > 0:
                ...
                if args.constraint.startswith('bertscore'):
                    ...
                    cosine = (output * orig_output).sum(1) / output.norm(2, 1) / orig_output.norm(2, 1)#形如a*b/|a||b|计算余弦值
            ...
            # Gradient step
            log_coeffs.grad.index_fill_(0, forbidden_indices.long(), 0)
            optimizer.step()
        ...
        save_path = 'Demonstration/'+ args.model +'_' + args.dataset+ '.txt'
        ...
        if args.dataset == 'mnli':
            ...
            #存储干净样本
            save_to_file(save_path, clean_premise  + clean_hypothesis )
            ...
            #存储干净样本
            save_to_file(save_path, clean_text)
            ...
            for j in range(args.gumbel_samples):
                ...
                if adv_logit.argmax() != label or j == args.gumbel_samples - 1:
                    if args.dataset == 'mnli':
                        ...
                        #存储对抗样本
                        save_to_file(save_path, adv_premise  + adv_hypothesis )
                        ...
                        #存储对抗样本
                        save_to_file(save_path, adv_text )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="mnli", type=str,
    ...
    parser.add_argument("--num_samples", default=60, type=int,
    ...
    parser.add_argument("--model", default="bert-base-uncased", type=str,
        ...
        "textattack/bert-base-uncased-MNLI", "bert-base-uncased", 
        "roberta-base", 'albert-base-v2', 'distilbert-base-uncased'],
        help="type of model")
    ...
    parser.add_argument("--initial_coeff", default=15, type=int,#ori=15
    ...
    parser.add_argument("--lam_sim", default=1, type=float,#Original=1
        help="embedding similarity regularizer")
    parser.add_argument("--lam_perp", default=1, type=float,#Original=1
